...
class PlayerManager:
    ...
    def Event_PlayerConnect(
        ...
        index += 1
        self.logger.info(
            f"OnConnect(name={name}, index={index}, userid={userid}"
            f", networkid={networkid}, address={address}, bot={bot})"
        )
    ...
    async def OnClientPostAdminCheckAsync(self, player: Player) -> None:
        ...
        player.OnClientPostAdminCheck(flag_bits=flag_bits, sourcemod_config=self.sourcemod_config)

    def Event_PlayerInfo(self, name: str, index: int, userid: int, networkid: str, bot: int) -> None:
        index += 1
        self.logger.info(f"OnInfo(name={name}, index={index}, userid={userid}, networkid={networkid}, bot={bot})")
        player = self.players[index]
    ...
    def Event_PlayerDisconnect(self, userid: int, reason: str, name: str, networkid: str, bot: int) -> None:
        self.player_count -= 1
        ...
        self.logger.info(
            f"OnDisconnect(index={player.index}, userid={userid}, reason={reason}"
            f", name={name}, networkid={networkid}, bot={bot})"
        )
